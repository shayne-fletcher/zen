 ...
 *)
Solutions(s) :
This version is credited to Joel Bjornson.
...
module Result : RESULT = struct
  ...
  module R = struct
    ...
    let get_ok = function
      | Ok x -> x
      | Error _ -> raise (
    Invalid_argument "get_ok"
    )

    let get_error = function
      | Ok _ -> raise (
    Invalid_argument "get_error"
    )
      | Error e -> e
    ...
    let reword_error_msg ?(replace = false
    ) f = function
      | Ok x  -> Ok x
      | Error (`
    Msg s)  when replace -> error_msg s
      | Error (`
    Msg s) -> Error (`
    Msg s)
    ...
    let trap_exn f x =
      ...
      | e ->
        Error (`
    Exn_trap (e, Printexc.get_raw_backtrace()
    ))
...
The following version is basically the original work published by Daniel C. Bünzli.
