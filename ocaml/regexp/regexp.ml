...
let rec (string_of_augmented_regexp :  augmented_regexp -> string) =
  fun x ->
...
let (string_of_follow_result : augmented_regexp * Int_set.t array * char option array -> string) =
 fun (e, follow, chars) ->
   let open Print_utils in
   Printf.sprintf "%s,\n%s, %s" 
...
let (accessible : state -> Int_set.t array -> char option array -> (char * Int_set.t) list) =
  fun s follow chars ->
...
let rec (compute_states : state list -> state list -> Int_set.t array -> char option array -> state array) =
...
let (dfa_of : augmented_regexp * Int_set.t array * char option array -> state array) =
  fun (e, follow, chars) ->
...
type t = (string -> char Lexical_analysis.Recognizer.remaining)
...
let re_match xpr s =
  let result = xpr s in
  match result with
  | Lexical_analysis.Recognizer.Remains [] -> true
  | _ -> false
