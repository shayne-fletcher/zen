...
and transl_exp0 e =
  match e.exp_desc with
  | Texp_ident(path, _, {val_kind = Val_prim{prim_name="%dyn_without_functions"|"%typeof"}}) ->
      Transldynamic.illegal_dyn_use e.exp_loc
  ...
  | Texp_apply({exp_desc =
                  Texp_ident(path, _,
                             {val_kind =
                                Val_prim{prim_name=("%use_ttype")}})},
               [(_, Some ttype); (_, Some e)])->
      Transldynamic.transl_use_ttype ttype e
  | Texp_apply({exp_desc = Texp_ident(path, _, {val_kind=Val_prim{prim_name="%typeof"}})},
               [(_, Some arg)]) ->
                 Transldynamic.transl_typeof arg
  ...
  | Texp_lazy e ->
      ...
      | Texp_ident(_, _, _) -> (* according to the type *)
          ...
          | Tpoly(_,_) | Tfield(_,_,_,_) | Tprop _ ->
              Lprim(Pmakeblock(Obj.forward_tag, Immutable), [transl_exp e])
          ...
          | Tconstr(_,_,_) ->
            ...
            (* lazy (Lazy.force e) ---> e *)
      | Texp_apply ({exp_desc=Texp_ident(_,_,{val_kind=Val_prim{prim_name="%lazy_force"}})}, [_, Some e0]) when
          can_simplify_lazy e0 ->
          transl_exp e0
  ...
  | Texp_typath l ->
      let block tag l = Lprim (Pmakeblock (tag, Immutable), l) in
      let cons hd tl = block 0 [hd; tl] in
      let str s = Lconst (Const_base (Const_string (s, None))) in
      let int n = Lconst (Const_base (Const_int n)) in
      let rec transl = function
        | [] -> int 0 (*nil*)
        | s :: rest -> cons (transl_step s) (transl rest)
      and transl_step = function
        ...
        | Ttypath_list nth -> block 3(*List_nth*) [transl_exp nth]
        | Ttypath_array nth -> block 4(*Array_nth*) [transl_exp nth]
      in
      transl l
  | Texp_unreachable ->
...
let () =
  Transldynamic.transl_exp_forward := transl_exp
