...
and transl_structure fields cc rootpath = function
  ...
  | Tstr_usettype ttype ->
      let id, ttype, rem = Transldynamic.transl_use_ttype_cont ttype (fun () -> transl_structure fields cc rootpath rem) in
      Llet (Strict, id, ttype, rem)
...
(* BEGIN LEXIFI *)
let transl_implementation module_name (str, cc) =
  Transldynamic.set_module_name module_name;
  let r = transl_implementation module_name (str, cc) in
  Transldynamic.add_mlfi_specific_to_module r
(* END LEXIFI *)
...
let rec defined_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_usettype _ -> defined_idents rem
...
let rec more_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_usettype _ -> more_idents rem
...
and all_idents = function
  ...
  | item :: rem ->
    ...
    | Tstr_usettype _ -> all_idents rem
    ...
    | item :: rem ->
        ...
        | Tstr_include incl ->
            ...
            Llet(Strict, mid,
                 ...
                 store_idents 0 ids)
        | Tstr_usettype ttype ->
            let id, ttype, rem = Transldynamic.transl_use_ttype_cont ttype (fun () -> transl_store rootpath subst rem) in
            Llet (Strict, id, subst_lambda subst ttype, rem)
        | Tstr_modtype _
        | Tstr_open _
        | Tstr_class_type _
        | Tstr_attribute _ ->
...
let transl_toplevel_item item =
  ...
  | Tstr_usettype ttype ->
      let id, ttype = Transldynamic.transl_use_ttype_toplevel ttype in
      toploop_setvalue id ttype
  | Tstr_modtype _
  | Tstr_open _
  | Tstr_primitive _
  | Tstr_type _
...
let transl_toplevel_item_and_close itm =
  close_toplevel_term
    (Transldynamic.add_mlfi_specific_to_module
       (transl_label_init (transl_toplevel_item itm)))
